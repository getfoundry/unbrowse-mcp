...
# 2. Get an ability's userAbilityId from the response

# 3. Execute the ability
curl -X POST 'http://localhost:4111/my/abilities/USER_ABILITY_ID/execute' \
  -H "Authorization: Bearer YOUR_API_KEY" \
...
```bash
Authorization: Bearer re_9pAvP8GsYqPy4oVAimoVZL4UJwgR1AsH4T4y4aorkXJX
```
...
```bash
curl 'http://localhost:4111/my/abilities' \
  -H "Authorization: Bearer re_9pAvP8GsYqPy4oVAimoVZL4UJwgR1AsH4T4y4aorkXJX"
```
...
**Key Fields:**
- `userAbilityId` - Use this to execute the ability
...
2. Check that the ability belongs to your account using `/my/abilities`
3. Make sure you're using the `userAbilityId` field, not `abilityId`
...
const result = await executeAbility(
  're_9pAvP8GsYqPy4oVAimoVZL4UJwgR1AsH4T4y4aorkXJX',
  'meowmeow',
  {
...
const executeAbilitySchema = z.object({
  abilityId: z.string().describe('The userAbilityId of the ability to execute'),
...
```bash
UNBROWSE_API_KEY=re_9pAvP8GsYqPy4oVAimoVZL4UJwgR1AsH4T4y4aorkXJX
...
**Common Mistakes:**
- Using `abilityId` instead of `userAbilityId` for execution
