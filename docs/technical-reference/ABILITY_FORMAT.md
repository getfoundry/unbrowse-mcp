...
| `output_schema` | object | ✅ | JSON Schema defining the output structure (auto-generated from actual API response) |
...
The `input_schema` field follows JSON Schema specification and defines what parameters the wrapper function accepts:
...
The `output_schema` field is **automatically generated** using [genson-js](https://www.npmjs.com/package/genson-js) from the actual API response during wrapper execution. This ensures the schema accurately reflects the real response structure.

### Generation Process

1. Wrapper is executed with test inputs during ingestion
2. Actual API response is captured
3. `genson-js` analyzes the response data structure
4. JSON Schema is automatically inferred and stored

### Example Output Schema

For an API returning an array of user objects:
...
| **Type Safety** | Provides complete type information for all fields |
| **Validation** | Can be used to validate responses in production |
| **Documentation** | Serves as living documentation of response structure |

### Common Output Types

| Response Type | Schema Example |
|--------------|----------------|
| **Object** | `{ "type": "object", "properties": {...} }` |
| **Array** | `{ "type": "array", "items": {...} }` |
| **Primitive** | `{ "type": "string" }` or `{ "type": "number" }` |
| **Mixed** | `{ "type": ["object", "null"] }` |

### Null Handling

If a wrapper execution fails or returns null, `output_schema` will be set to `null`:
...
This indicates the schema couldn't be inferred (e.g., authentication required, endpoint unavailable).
...
```json
{
  ...
  "static_headers": {
    ...
    "apikey": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZ0bnpocWF3aWxkaWVjYWFzYXB2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDQ3MzI1MDAsImV4cCI6MjA2MDMwODUwMH0.mHpzdECN-AZEv5LjcYDbPRnIYJp5ck47qaA84crvAdo",
    "authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZ0bnpocWF3aWxkaWVjYWFzYXB2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDQ3MzI1MDAsImV4cCI6MjA2MDMwODUwMH0.mHpzdECN-AZEv5LjcYDbPRnIYJp5ck47qaA84crvAdo",
...
- `description`
- `input_schema`
- `output_schema` (may be `null` if execution failed)
- `request_method`
- `request_url`
- `dependency_order`
...
### 3. Validating Response with Output Schema
...
// Execute the wrapper
const response = await wrapperFn({ token_symbol: '$fdry' });
