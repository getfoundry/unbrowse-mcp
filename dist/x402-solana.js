...
export class X402SolanaClient {
    ...
    constructor(config) {
        ...
        try {
            ...
            console.log(`[x402] Wallet initialized: ${this.keypair.publicKey.toBase58()}`);
        }
        catch (error) {
            throw new Error(`Invalid Solana private key: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
        if (config.rpcUrl) {
     ...
     */
    async createPaymentTransaction(requirement) {
        ...
        tx.add(ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 1000 }), ComputeBudgetProgram.setComputeUnitLimit({ units: 100000 }));
        ...
        for (const transfer of transfers) {
            const recipientAta = await getAssociatedTokenAddress(mint, transfer.recipient);
            ...
            try {
                ...
                tx.add(createAssociatedTokenAccountInstruction(payer, // payer
                recipientAta, // ata
            ...
            tx.add(createTransferInstruction(payerAta, // source
     ...
     */
    createPaymentHeader(tx, signature) {
        const paymentData = {
     ...
     */
    async processPaymentRequired(requirement) {
        try {
            console.log(`[x402] Processing payment: ${requirement.amountFormatted} to ${requirement.recipient}`);
            if (requirement.splits) {
                console.log(`[x402] Payment splits:`);
                for (const split of requirement.splits) {
                    console.log(`  - ${split.label}: ${split.percentage}% to ${split.recipient}`);
                }
            }
     ...
     */
    async getUsdcBalance(chain, mint) {
        try {
 ...
 */
export function parsePaymentRequirement(responseBody) {
    if (!responseBody?.payment) {
        return null;
    }
    const payment = responseBody.payment;
    if (payment.type !== "usdc" || payment.network !== "solana") {
        console.warn(`[x402] Unsupported payment type: ${payment.type}/${payment.network}`);
        return null;
    }
    return payment;
}
