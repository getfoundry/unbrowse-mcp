 ...
 */
import { createX402Client, parsePaymentRequirement, } from "./x402-solana.js";
 ...
 */
function transformAbilityResponse(apiAbility) {
    return {
...
export class UnbrowseApiClient {
     ...
     */
    async searchAbilities(query, limit = 12, domains) {
        const params = new URLSearchParams({
            q: query,
     ...
     */
    async searchPublicAbilities(query, limit = 30) {
     ...
     */
    async getAbilityWrapper(abilityId) {
     ...
     */
    async listCredentials(grouped = true) {
     ...
     */
    async storeCredentials(domain, credentials) {
     ...
     */
    async deleteCredentialsForDomain(domain) {
        const url = `${this.baseUrl}/my/credentials/${encodeURIComponent(domain)}`;
        const response = await this.fetchWithTimeout(url, {
            method: 'DELETE',
        });
     ...
     */
    async expireCredentials(domain) {
     ...
     */
    async executeAbility(abilityId, params, options = {}) {
 ...
 */
export function createApiClient(authToken) {
    // Auto-detect whether this is an API key or session token
    const isApiKey = authToken.startsWith("re_");
    return new UnbrowseApiClient(isApiKey ? { apiKey: authToken } : { sessionToken: authToken });
}
/**
...
export class UnbrowseX402Client {
     ...
     */
    async searchAbilities(query, limit = 12) {
        const params = new URLSearchParams({
            q: query,
     ...
     */
    getPaymentHistory(limit = 50, type) {
 ...
 */
export function createX402ApiClient(privateKey, rpcUrl) {
...
export function formatAbilityDescription(ability) {
    ...
    if (ability.dependency_order && ability.dependency_order.length > 0) {
        desc += `\n\n**Dependency Order:** This ability must be called AFTER: ${ability.dependency_order.map((depId) => `\`${depId}\``).join(" → ")}`;
    ...
    if (ability.dependencies?.missing &&
