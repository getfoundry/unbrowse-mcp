...
Sentry.init({
  dsn: "https://24df40b0c4210b58fbf1c9b560a1e69d@o4510271972245504.ingest.us.sentry.io/4510271973228544",
...
"Retrieves a paginated list of processed invoices from Zeemart's invoice management service, sorted by invoice date in descending order. Use this to fetch invoice history for financial reconciliation, payment tracking, or reporting. This endpoint supports pagination, sorting, and field selection to customize the response. Parameters: pageNumber (number, default 1) - Page number for pagination; pageSize (number, default 50) - Number of records per page; sortBy (string, default 'invoiceDate') - Field to sort by (e.g., 'invoiceDate', 'totalCharge'); sortOrder (string, default 'DESC') - Sort direction ('ASC' or 'DESC'); includeFields (string, comma-separated) - Specific fields to include (e.g., 'invoiceId,invoiceNum,totalCharge,paymentDueDate,supplier,status,createdBy,lastUpdatedBy,images,outlet,dueDate,invoiceType,paymentStatus,invoiceBuyerApproval,onPaymentPlan,partnerIntegration'). Examples: Fetch first 50 processed invoices sorted by date descending: {pageNumber: 1, pageSize: 50, sortBy: 'invoiceDate', sortOrder: 'DESC', includeFields: 'invoiceId,totalCharge,status'}; Get next page with custom fields: {pageNumber: 2, pageSize: 25, includeFields: 'supplier,outlet,paymentStatus'}; Retrieve all fields for detailed analysis: {pageNumber: 1, pageSize: 100, includeFields: 'all'}. This can fulfill queries like 'list recent processed invoices', 'get unpaid invoices sorted by due date', 'fetch invoice details for outlet OAMZQ', or 'paginate through invoice history for reporting'. Response includes pagination metadata (totalPages, totalRecords) and data array with invoice objects containing specified fields. Call this after get-account-outlets to filter by outlet or get-invoice-count-by-status to check totals before paginating. Supports multi-outlet accounts via authentication context; combine with get-single-invoice for detailed views of specific invoices or get-payment-terms for due date calculations."

"Retrieves detailed information for a specific Statement of Account (SOA) by its ID for a given company from Zeemart's payment management service. Use this to fetch comprehensive SOA details including invoice line items, payment history, due amounts, supplier information, and status for payment verification, dispute resolution, or detailed financial reconciliation. Parameters: soaId (required, string) - Unique SOA identifier like 'SABBMSOA0000015'; companyId (required, string) - Company identifier like 'CAGNI'. Examples: Get details for SOA 'SABBMSOA0000015' in company 'CAGNI': {soaId: 'SABBMSOA0000015', companyId: 'CAGNI'}; Use for viewing specific billing period with supplier. This can fulfill queries like 'Get full details of SOA SABBMSOA0000015', 'Fetch invoice breakdown for specific SOA', 'View payment history for SOA ID', or 'SOA document details for accounting'. Response includes full SOA object with line items, totals, dates, statuses, and attachments. Call after list-buyer-soa-by-supplier to drill down into specific entries, or combine with get-soa-summary-by-supplier for context. Requires authentication; useful for detailed payment workflows and audit trails."
